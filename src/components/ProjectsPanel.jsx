...
function ProjectsPanel({ isOpen, onClose, width = 263, onWidthChange, minWidth = 263, maxWidth = 600 }) {
  ...
  return (
    <aside
      className={`sounds-like-panel projects-panel ${isOpen ? 'open' : ''}${isOpen && width > minWidth ? ' sounds-like-panel--overlay' : ''}`}
      role="dialog"
    ...
    >
      ...
      <div className="sounds-like-panel-header">
        <div className="sounds-like-panel-title-row">
          <h2 className="sounds-like-panel-title">Projects Panel</h2>
        </div>
      ...
      <div className="sounds-like-panel-content">
        {PROJECTS_PANEL_ITEMS.map((title, i) => (
          <div
            key={`${title}-${i}`}
            className={`sounds-like-track ${width >= 440 ? 'sounds-like-track-expanded' : ''}`}
          >
            <div className="sounds-like-track-row">
              <button type="button" className="sounds-like-track-play" aria-label={`Open ${title}`}>
